...
The BOSSBase 1.01 dataset \cite{bas2011break} is a standard academic image library recognized in forensics research. 
...
While steganography can be applied for ethical purposes like protecting privacy and copyrights; it can also be used for malicious purposes like data exfiltration attacks and command and control communication in cyber-attacks. 
Studying steganalysis, or techniques for detecting steganographic communications, is a positive contribution as it helps forensic specialists in detecting hidden communication channels. 
Instead of using the research for developing new embedding algorithms that are difficult to be detected, this study strictly focuses on the detection of LSB steganography, which is an example of good security research ethics. 

To ensure scientific integrity, this study takes the importance of replicability into consideration. 
...
For the size of the embedding message 25\% ($r=0.25$), 50\% ($r=0.50$), and 75\% ($r=0.75$) of the embedding capacity were chosen. 
...
These were implemented in Python scripts from the theoretical sources listed in Table \ref{tab:lsb_methods}.
...
\begin {table} [h]
  ...
  \begin{tabular}{l p{5cm} p{3cm}} % defines number of columns and their layout 
    ...
    \midrule
    \verb|Sequential LSB| & LSB embedding in its simplest form.  & \cite{Chan2004}\\
    \midrule[0.5pt]
    \verb|Randomized LSB| & Use of PRNG for selecting embedding locations. & \cite{Sharp2001} \\
    \midrule[0.5pt]
    \texttt{LSB Matching ($\pm$1)} & Randomly adds or subtracts 1 from the pixel value instead of replacing it. & \cite{Ker2005}\\
    \midrule[0.5pt]
    \verb|Sobel Edge Adaptive LSB| & Identifies "noisy" pixels for embedding. & \cite{Luo2010, smitha2018sobel}\\
    \bottomrule[1.5pt]
  \end{tabular}
  \caption {LSB Methods in this Study}
  \label{tab:lsb_methods}
\end{table}
...
The network was trained using the Adamax optimizer \cite{Kingma2014}, since \textcite{Boroumand2019} found it reliable with a fast convergence relative to other alternatives. 
...
\begin{table}[h]
  ...
  \begin{tabular}{ll}
    ...
    \midrule
    Input size              & $256 \times 256$ grayscale \\
    Train / Val / Test      & 7000 / 1500 / 1500 \\
    Batch size              & 20 \\
    Optimizer               & Adamax \\
    Initial learning rate   & $1 \times 10^{-3}$ \\
    LR schedule             & $\times 0.1$ every 30 epochs \\
    Loss                    & NLLLoss (LogSoftmax output) \\
    Augmentation            & Random rotation up to $90^\circ$ \\
...
To ensure full transparency and to facilitate the reproducibility of the experimental pipeline, all custom scripts, model architectures, 
and data processing pipelines developed for this thesis have been made publicly available. 

The primary repository containing the overall experimental pipeline and the steganographic embedding methods can be 
found in the main project repository \cite{GullbrandssonPakos2026project}. 
The implementations of the steganalysis methods, SRNet \cite{GullbrandssonPakos2026srnet} and RS-analysis \cite{GullbrandssonPakos2026rs}, utilized pre-existing implementations as a base, which were forked and modified to fit this 
experiment. These are available as subrepositories within the main project repository and can be accessed independently. The creators of the original repositories are properly attributed within the respective forks.

Readers and researchers are encouraged to consult these repositories for exact code-level details regarding the hyperparameters, 
preprocessing steps, and evaluation loops utilized in this study.
...
This study does not include more modern steganographic methods like WOW, HUGO, MiPOD, or S-UNIWARD. This was done to limit the study, but decreases its usability in a modern context.
...
In DSR, the focus is on developing and assessing a new artifact, such as a new detection algorithm, an improved CNN framework, 
or an integrated detection scheme based on statistics and deep learning, with the designed artifact being the key research contribution. 
In this process, knowledge creation occurs by repeated building and testing, followed by communicating design knowledge as the research outcome.
...
To meet the requirements of academic integrity, the authors wish to acknowledge that AI tools were used in the process of writing this thesis in a 
supporting role. 
In particular, AI programs were used for: verifying the spelling and grammar in sentences, proposing rewrites aimed at 
enhancing the readability of sentences written by the authors, helping find and pinpoint source literature, which was then reviewed, 
validated, and referenced by the authors directly, and giving support in coding the experiment pipeline, involving debugging and syntax.
